...
class StrategicCustomer(Env):
    ...
    def __init__(self):
        ...
        self.max_price = min([self.vendor_model.predict([s], deterministic=True)[0][0] for s in range(config.week_length)])
    ...
    def step(self, action, simulation_mode = False):
        ...
        price = self.vendor_model.predict([self.s[0]], deterministic=True)[0][0]
        ...
        # state_index = 1 + config.strategic_enter_waiting_pool
