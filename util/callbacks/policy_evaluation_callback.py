...
class PolicyEvaluationCallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
        ...
        # Store the last prices as a 'prediction' for the customer
        # Customer.last_prices = actual_prices

        state = np.array(self.locals['obs_tensor'][0])

        actual_prices = [self.model.predict([s, *state[1:]], deterministic=True)[0][0] for s in range(config.week_length)]
        ...
        wandb.log({'seasonal_Price_Diff': calculate_difference(actual_prices, self.optimal_prices)}, step=self.n_calls)
        wandb.log({'seasonal_Profit_Diff': calculate_difference(expected_profits_per_customer, self.optimal_profits)}, step=self.n_calls)
...
inner_callback = PolicyEvaluationCallback()

policy_evaluation_callback = EveryNTimesteps(n_steps=config.episode_length * config.peval_cb_n_episodes, callback=inner_callback)